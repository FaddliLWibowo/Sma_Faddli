<script type="text/javascript">
	...
	function cariSiswa(namaSiswa) {
		if(namaSiswa.length == 0) {
			...
			$.post("<?php echo base_url(); ?>tatausaha/tampil", {kirimNama: ""+namaSiswa+""}, function(data){
				if(data.length >0) {
					$('#hasilPencarian').fadeIn();
					$('#dataPencarian').html(data);
				}
				else {$('#hasilPencarian').hide();}
			});
		}
	} 
...
<body>
	<div>
		...
		<form>
			<div style="width:400px; margin:0px left;">
				<input type="text" size="30" id="namaSiswa" onkeyup="cariSiswa(this.value);" onblur="pilih();"  
...
<table border="2" cellpadding="1" cellspacing="1" width="100%" style=" border-collapse: collapse;" class="record" bordercolor="#CCCCCC">
			...
			$no = 1;
				...
				{
				foreach($data_siswa->result_array() as $db)
					{
				?>
				<tr>
					<td><?php echo $no; ?></td>
